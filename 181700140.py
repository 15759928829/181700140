...
while 1:  # 循环直到读到END
	try:  # 尝试 然后捕获异常
		...
		break
	grade = str[0]
	teststr = split_string(str)
	address = teststr['address']
	add = dict()
	add['name'] = teststr['name']
	add['telephone'] = teststr['telephone']
	county = ['区', '市', '县']
	street = ['街道', '镇', '乡']
	road = ['路', '巷', '道', '街', '弄', '胡同']
	for i in data:
	...
	sss=dict()
	sss['姓名']=add['name']
	sss['手机']=add['telephone']
	sss['地址']=result

	ddd = json.dumps(sss, ensure_ascii=False)
	print(ddd)